...
static void ble_tank_select_cb(lv_event_t* e) {
    lv_obj_t* btn   = lv_event_get_target_obj(e);
    const char* mac = (const char*)lv_obj_get_user_data(btn);
    lv_textarea_set_text(bl_ctx.ta_tank_mac, mac ? mac : "");
    lv_obj_t* ov   = lv_obj_get_parent(lv_obj_get_parent(btn));
    lv_obj_t* prev = (lv_obj_t*)lv_obj_get_user_data(ov);
    lv_screen_load(prev);
    lv_obj_delete(ov);
...
static void ble_scan_tank_cb(lv_event_t*) {
    lv_obj_t* list = nullptr;
    lv_obj_t* sp   = nullptr;
    make_scan_overlay(bl_ctx.scr, t(TK::SCAN_TANK), &list, &sp);
    auto* ctx      = (BleScanCtx*)malloc(sizeof(BleScanCtx));
    ctx->list      = list;
    ctx->spinner   = sp;
    ctx->select_cb = ble_tank_select_cb;
    // No mfr-id filter — BTHome devices carry service-data, not mfr-data,
    // so they'd be filtered out by victron_only=true.
    bleDiscoveryScan(false, ble_scan_done, ctx);
}
