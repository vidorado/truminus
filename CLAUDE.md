...
- **`MBEDTLS_ERR_SSL_ALLOC_FAILED (-0x3B00)` + `assert ... sdio_rx_get_buffer ... sdio_drv.c:953` are the same bug: internal DRAM exhaustion.** When the WSS tunnel reconnects right after `IP_EVENT_STA_GOT_IP`, the TLS handshake tries to allocate large chunks; if mbedtls is pinned to internal DRAM the heap drains, mbedtls returns `-0x3B00`, and a moment later the C6 SDIO RX path can't get a DMA-capable buffer and asserts. The knobs that prevent it are pinned in `sdkconfig.defaults` with a comment: `CONFIG_MBEDTLS_DEFAULT_MEM_ALLOC=y` (was `INTERNAL_MEM_ALLOC`) and `CONFIG_SPIRAM_MALLOC_ALWAYSINTERNAL=2048` (was the IDF default 16384) — together these route large mbedtls buffers to PSRAM, so the internal DRAM no longer drains. `CONFIG_MBEDTLS_SSL_IN_CONTENT_LEN` is kept at the **16384** TLS maximum (an earlier 4096 trim broke self-OTA — the GitHub release CDN sends full-size TLS records, so reading the response failed with mbedtls `-0x0087` / "Complete headers were not received"); with the two mem-alloc knobs above this 16 KB buffer lands in PSRAM and does not reintroduce the cascade. `CONFIG_MBEDTLS_ASYMMETRIC_CONTENT_LEN=y` keeps the OUT buffer at 4096. Don't disable the tunnel as a "fix" — that's masking the heap, not solving it. See `.claude/skills/pio-idf-p4/SKILL.md` §"DRAM exhaustion cascade".
